...
with torch.no_grad():
    torch_out = torch.onnx._export(model, x, 'model.onnx', input_names=['data'], output_names=['output'], dynamic_axes=dynamic_axes, opset_version=15)

from onnxruntime.quantization.quantize import quantize_dynamic, QuantType
quantized_model = quantize_dynamic("model.onnx", "model.quant.onnx", weight_type=QuantType.QInt8)
